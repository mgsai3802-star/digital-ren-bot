...
# အရင်ရှိပြီးသား ID များကို Recovery လုပ်ရန်စာရင်း
OLD_IDS = [
...
# --- Forward & Reply System (Media Supports: Text, Photo, Video, Voice, Audio, Doc) ---
    ...
    if message.chat.id != ADMIN_ID:
        ...
        try:
            ...
            if message.content_type == 'text':
                ...
                bot.send_photo(target_id, message.photo[-1].file_id, caption=f"👨‍💻 **Admin ပုံပို့လိုက်သည်:**\n\n{message.caption or ''}", reply_markup=main_menu())
            elif message.content_type == 'video':
                bot.send_video(target_id, message.video.file_id, caption=f"👨‍💻 **Admin ဗီဒီယိုပို့လိုက်သည်:**\n\n{message.caption or ''}", reply_markup=main_menu())
            elif message.content_type == 'voice':
                bot.send_voice(target_id, message.voice.file_id, reply_markup=main_menu())
            elif message.content_type == 'document':
...
if __name__ == "__main__":
    # Bot စတင်သည်နှင့် ID ဟောင်းများကို အလိုအလျောက် သိမ်းဆည်းမည်
    recover_old_ids()
